...
How to get Flaremender running, on Cloudflare or on your laptop. For what it is and
...
| ------------------------------ | ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Workers Paid**, $5 a month   | Dynamic Workers, which sandbox every test script  | **Required.** Dynamic Workers are Paid-only. The Free plan's 10 ms CPU limit per request is also too small for a generation turn                  |
| **R2 enabled**                 | screenshots, traces and logs                      | Enable once in the dashboard under R2                                                                                                             |
| **An LLM provider**            | generation, exploration and chat                  | Workers AI is the default and needs no key. Anthropic, OpenAI or Google produce better scripts. Keys can be set per instance or per organization  |
...
| Workers Logs and Traces        | every run, turn and model call, as a waterfall    | On by default in `wrangler.jsonc`. Free until October 1, 2026. After that, spans share the logs quota, which is 20 million events a month on Paid |
...
| ---------------------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| Workers Paid           | the plan itself                                    | $5 a month                                                                                              |
| Browser Rendering      | 10 minutes a day on Free, 10 hours a month on Paid | $0.09 per browser-hour                                                                                  |
| Dynamic Workers        | 1,000 unique Workers a month                       | $0.002 per unique Worker per day. A test script that changes counts as a new one                        |
| Workers AI             | 10,000 Neurons a day                               | $0.011 per 1,000 Neurons                                                                                |
...
Watch the first few runs before you put anything on a schedule.
...
Durable Objects and Workers AI binding, and asks for the secrets below. Workflows,
...
`Host` header, pin the public origin with `wrangler secret put BETTER_AUTH_URL`.
...
A project's environments hold the base URL a run points at and the credentials it
may read. Values are write-only. They are encrypted at rest, and only the last four
...
Worker secret, then redeploy. Your account keeps working, and so does anyone holding
a pending invitation, so you can still add teammates from Organization settings.
...
plan. Workers AI is not free locally. `wrangler.jsonc` marks the `AI` binding
`remote`, so model calls in local development reach your real account and are
billed. Set an Anthropic, OpenAI or Google key in the admin console to avoid that.
...
regenerates the images in the README from that data.

### Database

The schema lives in `src/db/schema/`. `auth.ts` is generated. Never edit it by hand.
...
`src/lib/auth/auth.ts`. Keep their plugin lists in sync, or the generated schema
drifts from what the app needs.

Use migrations for an existing database. The baseline also adopts databases created
with the original schema without deleting their history. Do not run `db:push` before
migrations. Pushing the new columns first would bypass the readiness and historical
verification backfills.
