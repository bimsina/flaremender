...
id, the dashboard link and the reason a generation stopped. A low number can be
...
test. Sonnet's miss was a generated script that did not replay green. Re-run these
before you trust them for a decision. A single run has noise in it.

**Through AI Gateway.** The same day, the five Guestbook scenarios with the OpenAI
key forwarded over the gateway, tagged `--tag gateway`, scored 80% against 100%
direct. One generated script did not replay green. The gateway's OpenAI route uses
the chat-completions API, which refuses tool calls with reasoning enabled, so
...
It signs in as the demo account from `pnpm demo:seed`. Override the account with
`FLAREMENDER_EVAL_EMAIL` and `FLAREMENDER_EVAL_PASSWORD`. It starts the example apps
...
[API](webhooks.md#create-and-generate-tests), the same way a script on your machine
would.
...
`{provider}:{slug}`. The provider needs a key on the instance or in the
...
control. The point is that a run today and a run next month measure the same thing.
