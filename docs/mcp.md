...
Flaremender is an MCP server. Connect it to Claude, Cursor, ChatGPT, Claude Code or
any other assistant that speaks the Model Context Protocol, and that assistant can
list your projects, write tests, run them, read the failures, and ask for repairs,
all as you, inside one organization you choose.
...
assistant discovers the authorization server on its own, sends you to a consent page,
and gets a token that expires after an hour and refreshes itself.
...
can reach exactly what you can reach and nothing more, and rows it creates are
...
with `get_run`. Every answer carries a `dashboardUrl`, so the assistant can hand you a
link to the full picture.

Tool errors come back as ordinary results with `isError` set and a sentence saying
what went wrong, the same sentence the dashboard would show. The assistant reads
it and adjusts; a missing environment variable, a test that has no script yet, or a
run that cannot be repaired because a newer script exists are all reported that way.
...
(keys are prefixed `grant:`). A "Connected assistants" page in Organization settings,
listing grants with a revoke button, is on the roadmap.
...
and paste the id it prints into `kv_namespaces` in `wrangler.jsonc`. Locally, dev
mode creates one under `.wrangler/` on its own.
...
`createMcpHandler` and `@modelcontextprotocol/server`, reusing the same actions the
dashboard and the webhook API call; nothing here has its own path into the
database. `src/mcp/authorize.ts` is the consent page, backed by the Better Auth
...
connection, for which organization, with which scopes, and where the instance lives,
which is how tools know what to check and what links to hand back.
