...
has repaired something, through a signed webhook, a Slack or Discord channel, or an
email. Destinations live under a project's Settings tab and are managed by the
...
failures listed, so a scheduled suite of forty tests is one message, not forty. Draft
checks and the verification runs behind generation and repair never notify.
...
Create an incoming webhook in Slack (Apps → Incoming Webhooks) or a channel webhook
in Discord (Channel settings → Integrations → Webhooks), then add a destination of
that kind with the URL. Messages carry the headline, the environment, the first line
of the error and a link to the run. Use **Send test** on the row to see one.

## Signed webhooks

A `webhook` destination receives every event as JSON, `POST`ed to your URL with these
headers:
...
The signature is an HMAC-SHA256 of `<t>.<raw body>` under the secret shown once when
the destination was created. Verify it before trusting the body, and reject anything
...
Each delivery is attempted twice, ten seconds apart on a non-2xx response, and
logged on the destination row. Links in the payload point at the origin the
destination was created from.

## Email

Email goes through [Cloudflare Email Service](https://developers.cloudflare.com/email-service/)
and is off until the instance opts in, because the binding needs an account with
Email Service set up:

1. Onboard a sending domain under Compute → Email Service → Email Sending. Until a
   domain is onboarded, Email Service only delivers to
...
Without the binding, an email destination can still be created; every delivery to
it is logged as failed with a message that says what is missing.

In local development the binding is simulated: `wrangler dev` writes each message to
a file under `.wrangler/` and prints its path instead of sending.
