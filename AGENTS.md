...
CI runs exactly these plus `pnpm build`. Do not open a PR that fails any of them.
...
The block below is generated by `@tanstack/intent` and lists library guidance you
can load on demand. Leave its markers in place.
