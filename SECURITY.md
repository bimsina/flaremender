...
an AI agent. Report anything that undermines either.
...
- **Script isolation.** Scripts run in a Dynamic Worker with `globalOutbound: null`
  and no D1, R2 or AI bindings.
...
This is a spare-time project with one maintainer, so do not expect a same-day reply.
